...
(function(module) {
  ...
  */
  ...
  Article.numWordsAll = () => {
    return Article.all.map(x => x.body.split(' ').length).reduce((count, value) => count + value);
  };
  ...
  Article.allAuthors = () => {
    return Article.all.map(x => x.author).reduce((names, name) => {
  ...
  Article.numWordsByAuthor = () => {
    return Article.allAuthors().map(author => {
      return {
        name: author,
        numWords: Article.all.filter(x => x.author === author)
          .map(x => x.body.match(/(^|\s)\w+/g).length)
          .reduce((a,b) => a +b)
      }
    })
  };
